...
loop_interval = 15                                                      # Интервал работы цикла
...
while True:
    ...
    def main():
        ...
        for i in range(len(values)):
            values[i].append(float(values[i][2])*d['Доллар США'])           # Добавлена сумма в рублях
            values[i][0] = int(values[i][0])
            values[i][1] = int(values[i][1])
            values[i][2] = int(values[i][2])


        valuesTupled = list(map(tuple, values))                             # Получаем кортеж
